...
if annot:
    import os, sys
    sys.path.insert(0, os.path.abspath('venv/lib/python2.7'))
...
ROOT = os.path.expanduser(os.path.abspath(__file__))
DB = "sqlite:///%s/annotator.sqlite" % os.getcwd()
MOUNTPOINT = "/store"
AModel.metadata.bind = DB
# Create tables
...
## XXX
print "HACKING IN JAVASCRIPT PATHS"
forceload = (
    ##("jquery", "lib/bower_components/jquery/jquery.min.js"),
    ("annotator", "lib/annotator.1.2.7/annotator.min.js"),
    ("annotator_store", "lib/annotator.1.2.7/store.js"),
)
for key, path in forceload:
    ## JavaScriptLoader.JS[key] = os.path.abspath(path)
    print "loading", path
    JavaScriptLoader.JS[key] = "../../calibre-viewer-annotation/" + path
JavaScriptLoader.ORDER = JavaScriptLoader.ORDER + tuple([pair[0] for pair in forceload])
...
class Document(QWebPage):  # {{{
    ...
    def AJAX(self, q_url, q_data_string):
        ...
        ebookviewer = self.parent().manager
        ...
        if request_type == "GET":
            ...
            if jsr["data"]:
                ...
                ## but who cares now
                ## self.bridge_value = AStore.search_annotations(uri = jsr["data"]["uri"])
                ...
                if annot_resultset["total"] > 0:
                    ...
                    self.bridge_value = json.dumps(res)
                ## self.bridge_value = AStore.index()
            else:
                ## this is the vanilla action: get all
                self.bridge_value = AStore.index()
            should_update_documentview = True
            print self.bridge_value
        #create:  POST
        ...
        if should_update_documentview:
            self.mainFrame().evaluateJavaScript("if(window.evil_callback) {console.log('executing evil callback'); window.evil_callback(JSON.parse(py_bridge.value)); window.evil_callback = null; }")
        if should_update_annotation_list:
            ebookviewer.ntoc_model.update_current_annotation_list()
    ...
    def load_javascript_libraries(self):
        ...
        ## re-init annotator
        print "CURRENT PAGE"
        ## print self.page_position.current_pos
        parent = self.parent()
        ebookviewer = parent.manager

        if ebookviewer.iterator:
            ## current_spine = ebookviewer.iterator.spine[ebookviewer.current_index]
            current_spine = parent.last_loaded_path
            base_path, href = os.path.split(current_spine)
            frag = (href + "#").split("#")[1]
            print "<<< <<< <<< <<< <<< base_path, href, frag >>> >>> >>> >>> >>>"
            print base_path, href, frag
            self.javascript(u"""
annotator = $(document.body).annotator()
...
class DocumentView(QWebView):  # {{{
    ...
    def load_path(self, path, pos=0.0):
        ...
        if annot:
            ...
            newhtml = AnnotationProcessor.process_html(self, str(myhtml).decode("utf-8"))
        ...
        if False:
            ...
            if os.path.exists("/tmp/runtelnet"):
                def quit():
                    reactor.stop()
                context = locals()
