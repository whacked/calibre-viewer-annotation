...
from documentview import AStore
from PyQt4.Qt import QStandardItem, QStandardItemModel
from calibre.ebooks.metadata.toc import TOC as MetaTOC
from calibre.gui2.viewer.toc import TOCItem
import json
...
class EbookViewer(MainWindow, Ui_EbookViewer):
    ...
    def __init__(self, pathtoebook=None, debug_javascript=False, open_at=None,
        ...
        self.action_table_of_ncontents.setCheckable(True)
        self.toc.setMinimumWidth(80)
        self.ntoc.setMinimumWidth(80)
        ...
        self.action_table_of_ncontents.toggled[bool].connect(self.set_ntoc_visible)
        ...
        self.ntoc.pressed[QModelIndex].connect(self.ntoc_clicked)
        ...
        self.ntoc.setCursor(Qt.PointingHandCursor)
    ...
    def set_ntoc_visible(self, yes):
        self.ntoc.setVisible(yes)
    ...
    def save_state(self):
        ...
        vprefs.set('viewer_ntoc_isvisible', bool(self.toc.isVisible()))
        if self.toc.isVisible():
            vprefs.set('viewer_splitter_state',
                bytearray(self.splitter.saveState()))
        vprefs.set('viewer_ntoc_isvisible', bool(self.ntoc.isVisible()))
        if self.ntoc.isVisible():
            vprefs.set('viewer_splitter_state',
                bytearray(self.splitter.saveState()))
    ...
    def ntoc_clicked(self, index, force=False):
        if force or QApplication.mouseButtons() & Qt.LeftButton:
            item = self.ntoc_model.itemFromIndex(index)
            if item.bookmark:
                self.goto_bookmark(item.bookmark)
            else:
                return error_dialog(self, _('No such location'),
                        _('The location pointed to by this item'
    ...
    ## XXX check this out for updating view state when clicking a note entry
    ...
    def load_ebook(self, pathtoebook, open_at=None):
        ...
        if worker.exception is not None:
            ...
            # FIXME FIXME FIXME
            self.ntoc_model = AnnotationTOC(self.iterator.spine)
            self.ntoc.setModel(self.ntoc_model)
    ...
    def read_settings(self):
        ...
        if c.remember_window_size:
            ...
            self.show_ntoc_on_open = vprefs.get('viewer_ntoc_isvisible', False)
