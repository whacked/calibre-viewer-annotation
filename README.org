...
* adding annotation (read and write) capability to calibre's ebook-viewer
  ...
  sporadic work in progress. last updated on 2016-02-07 *(BREAKS EARLIER VERSION, see below)*
...
* installing+using the plugin
  ...
  *this part is not yet tested following the model update*
...
* breaking changes
  ...
  while breaking, the data model is mostly the same.
  
  if you actually need to migrate, consider using a csv export then import, or some combination of =ALTER= on your database

** 2016-02-09

   - (TODO) supercede common =Annotation.text= usage with =Annotation.quote=
   - elixir removed, thus elixir-generated table names are probably no longer valid (=annotator_model_range= etc)
