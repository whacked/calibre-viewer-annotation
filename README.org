...
***** sqlite3
      ...
      this must be built for your platform to be used in node-webkit. For
      linux this may work:

      #+BEGIN_SRC sh :eval never
      npm install sqlite3 --build-from-source --runtime=node-webkit --target_arch=x64 --target="0.19.3"
      #+END_SRC
